...
**唯一的例外：為了限定「資源持有範圍」而抽。** 有些語言用 `defer` / `using` / `try-with-resources` 把「拿了要還」綁在**函式或區塊的結尾**——鎖、交易、連線、檔案。當一段程式需要「持有到這裡為止，然後在下一段之前一定要放掉」，抽成一個 method 就是在畫那個範圍，內聯回去等於改成手動歸還，而手動歸還會在每個 `return`、每個 panic 路徑上漏掉一次。

這不是「看起來整齊」，是**正確性**：

```
✅ takeDeparted()   拿鎖 → 挑出要退場的 → defer 還鎖
   （回到呼叫端，鎖已經放掉，才去做那件會等很久的事）
❌ 內聯之後         拿鎖 …… 一路手動記得在 4 個 return 前都要還
```

判準：**內聯之後會不會出現手動歸還？** 會，就留著它。留的時候在註解寫清楚它圈的是什麼範圍、為什麼不能一路持有——否則下一個人只會看到「一個只被呼叫一次的 private method」。

## goroutine / 執行緒生命週期物件不是 Domain Model

長駐服務常需要一個物件來拿著「一件正在跑的工作」：取消函式、完成訊號、它服務的對象。**這種物件不是 entity、不是 Domain Model、不是 VO**——它拿的是執行機制，不是領域資料，`models/` 底下沒有它的位置。

- 放在**它所屬的那個 Domain Service 旁邊**（同一層、獨立檔案、一檔一型別）。
- **不加 `Domain` / `Vo` / `Dto` 後綴**——加了會讓人以為它是領域模型、可以放進 `models/`、可以帶業務規則。
- 只拿執行機制與它要通知的對象；**任何業務規則仍然要放進 Domain Model**，由這個物件去呼叫。

判準：**拿掉取消函式與完成訊號之後，還剩下領域概念嗎？** 剩下了，就把那部分抽成 Domain Model；只剩機制，它就留在 service 旁邊。
