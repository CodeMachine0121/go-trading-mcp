...
- 合約那一條線的每一件事都是一個能力，共 **15 件**，能力從 51 件變成 **66 件**。
...
Scenario: 依刻度彙總查合約 K 線
  Given 使用者要看 BTCUSDT 永續合約這一週的圖
  When 助理用查合約 K 線序列的能力,說可顯示 100 根
  Then 問到合約那一條線的彙總序列,帶著合約標的、起訖時間與可顯示根數

Scenario: 查合約 K 線序列的說明寫出兩種說法只能挑一種
  When 助理讀查合約 K 線序列的說明
  Then 說明寫著彙總刻度與可顯示根數兩者只能給一個
...
Scenario: 能力清單剛好多了這十四件
  Given 這一刀之前的五十一件能力
  When 看能力清單
  Then 清單是那五十一件加上合約的十五件,不多也不少,而且每一件要不要身分與原本相同
...
- 查資料的每一件合約能力，說明都寫出交易服務**會在什麼情況拒絕**（區間超過單次上限、結束早於開始、指名的那一筆不存在），助理不必靠被拒絕才發現。
