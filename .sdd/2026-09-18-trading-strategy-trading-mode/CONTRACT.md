...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| AC-01 | 建立一份現貨型交易策略 | 那個值原樣出現在送往交易服務的 body 裡 | `tool_catalog_strategy.go:95`（`bodyParameter` ⇒ `ToolParameterLocationBody`） | `tool_catalog_test.go:188`（`trading_create_trading_strategy` 那一輪） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-02 | 改掉一份交易策略的交易模式 | 同上 | 同上——改寫吃的是同一份 `tradingStrategyWriteParameters()` | `tool_catalog_test.go:188`（`trading_update_trading_strategy` 那一輪） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-03 | 兩支共用同一份欄位清單 | 交易模式在兩支上一字不差；改寫多出來的只有路徑上的識別碼 | `tool_catalog_strategy.go:121`／`:139`（兩處都呼叫同一個函式，未改動） | `tool_catalog_test.go:188`（table-driven 跑兩支，**同一組斷言**） | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-05 | 交易模式不是必填 | `IsRequired` 為假 | `tool_catalog_strategy.go:95` 末參數 `false` | `tool_catalog_test.go:205` | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-07 | 說明講得出我該怎麼挑 | 兩個拼法的意思、不給會怎樣、不能放空時給哪一個 | `tool_catalog_strategy.go:95-101` | `tool_catalog_test.go:197-202`（四條：`spot`、`longShort`、「省略即 longShort」、「不能放空」） | asserts-oracle | produces-oracle | ✅ conforms |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| AC-08 | 那件能力沒有交易模式這一欄 | 欄位清單裡沒有 `tradingMode` | `tool_catalog.go:93` 的共用清單已移出它；`tool_catalog_strategy.go:193` 只吃那份共用清單，不附加 | `tool_catalog_test.go:218`（斷言**沒有**那一欄） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-09 | 說明講出為什麼沒有 | 交易模式取自那一份交易策略，這裡不必也不能再說一次 | `tool_catalog_strategy.go:184` | `tool_catalog_test.go:223`（斷言說明含「交易模式」） | asserts-oracle | produces-oracle | ✅ conforms |
| AC-10 | 說明講出要改的話去哪裡改 | 指出去 `trading_update_trading_strategy` 改那一份的 `tradingMode` | `tool_catalog_strategy.go:186-187` | `tool_catalog_test.go:224-225`（斷言說明含 `trading_update_trading_strategy` 與「不能放空」） | asserts-oracle | produces-oracle | ✅ conforms |

### US-03 — 重演一支策略腳本那條路一個字都沒變

| ID | Clause | Oracle | Implementation | Test | Test audit | Code audit | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| AC-11 | 那件能力仍然有交易模式，選填，說明一字不差 | 有那一欄、`IsRequired` 為假、說明與切片前相同 | `tool_catalog_strategy.go:177`——**說明字串逐字搬過來**，一個字都沒改 | `tool_catalog_test.go:235-237` | asserts-oracle | produces-oracle | ✅ conforms |
| AC-12 | 該共用的每一欄仍然共用 | 除了 `tradingMode` 之外，回測共通的每一欄在兩支上一字不差 | 兩支仍然都吃 `backtestParameters()`（`:165`、`:193`） | `tool_catalog_test.go:245-256`（逐欄比對：交易策略那一支的每一欄，除路徑識別碼外，腳本那一支都有，而且**整個欄位宣告相等**——只比名字會放過一份抄走後漂了一句話的複本） | asserts-oracle | produces-oracle | ✅ conforms |

### Core Business Rules（PRD §4）

| ID | Clause | Oracle | Implementation | Test | Test audit | Code audit | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| BR-01 | `tradingMode` 只出現在三處 | 建交易策略、改交易策略、重演一支策略腳本 | `grep tradingMode` 命中兩個 `bodyParameter`（`:95` 供建立與改寫共用、`:177` 給腳本回測）＋一句說明文字（`:187`） | `tool_catalog_test.go:188`（兩支有）、`:218`（一支沒有）、`:235`（一支有） | asserts-oracle | produces-oracle | ✅ conforms |
| BR-02 | 重演一份交易策略沒有它 | 那一份自己記著 | `tool_catalog_strategy.go:193` 不附加 | `tool_catalog_test.go:218` | asserts-oracle | produces-oracle | ✅ conforms |
| BR-03 | 選填，不補預設值 | 省略時不進 body；連接器不寫第二份預設值 | 末參數 `false`；沒有任何一處寫 `longShort` 當預設值（那個字只出現在說明句子裡） | `tool_catalog_test.go:205`、`:236` | asserts-oracle | produces-oracle | ✅ conforms |
| BR-04 | 不驗證取值 | 認不得的值照送 | 沒有任何比對取值的程式碼 | — | no-test | produces-oracle | 🟡 partial |
| BR-05 | 建立與改寫共用同一份欄位清單 | 沒有哪個欄位只在一條路上能填 | `tool_catalog_strategy.go:121`／`:139`（未改動） | `tool_catalog_test.go:188`（同一組斷言跑兩支） | asserts-oracle | produces-oracle | ✅ conforms |
| BR-06 | 兩支回測仍然共用真的共用的那幾欄 | 拆開的只有 `tradingMode` | `backtestParameters()` 仍然是兩支的來源 | `tool_catalog_test.go:245-256` | asserts-oracle | produces-oracle | ✅ conforms |
| BR-07 | 說明是助理唯一的依據 | 講出兩個拼法、省略的後果、不能放空對應哪一個 | `tool_catalog_strategy.go:95-101` | `tool_catalog_test.go:197-202` | asserts-oracle | produces-oracle | ✅ conforms |
...
| ID | Clause | Oracle | Implementation | Test | Test audit | Code audit | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| EC-01 | 助理在重演一份交易策略時硬塞 `tradingMode` | 那個欄位不在清單裡，連接器不會把它放進 body | `BuildRequest` 只送**宣告過的**欄位（未改動的既有行為） | `tool_catalog_test.go:218`（那一欄不存在）＋`:119`（切片前既有，守著 `BuildRequest` 的行為） | asserts-oracle | produces-oracle | 🟡 partial |
...
| ID | Clause | Oracle | Implementation | Test | Test audit | Code audit | Status |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| NFR-01 | `trading_backtest_strategy_script` 的欄位與說明一字不差 | 逐字 | `tool_catalog_strategy.go:177` 的說明字串**逐字搬過來** | `tool_catalog_test.go:235`、`:245`；切片前既有的每一條仍綠 | asserts-oracle | produces-oracle | ✅ conforms |
...
| :--- | :--- | :--- | :--- | :--- |
| 1 | 新增兩個測試輔助函式 `abilityNamed`／`boxNamed` | `tool_catalog_test.go:159`／`:174` | 無 PRD 條款 | ⚠️ 良性。只服務測試，且**兩個都被三條測試共用**。沒有它們，每一條測試都要在讀者面前走過整份清單 |
