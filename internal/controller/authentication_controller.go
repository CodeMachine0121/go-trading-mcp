...
func (authenticationController *AuthenticationController) RegisterOn(server *mcp.Server) {
	server.AddTool(&mcp.Tool{
		...
		Description: "用電子郵件與密碼登入交易服務，把身分交給這個外掛保管。" +
			...
			"\n\n**登入成功不代表這個帳號已經能用。** 交易服務要求帳號先經人工開通；" +
			"還沒開通的帳號登得進來，但其他每一件能力都會被拒絕，並在拒絕裡說出要寄信去哪、" +
			"主旨怎麼寫。遇到那種拒絕時，**把那個信箱與那串主旨原封不動轉告使用者**，" +
			"請他自己寄出那封信——不要改寫主旨（收信的人靠它認出是誰在申請），" +
			"也不要重登或換新登入（那份登入好好的，重來一次不會有任何改變）。",
